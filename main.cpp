...
void prodactive(string terminals, vector<string>* RProd, vector<string>* LProd) //for type 2 grammar
{
    ...
    while(P != PreviousP)
    {
        ...
        for(int i = 0; i < RightRule.size(); i++)
        {
            ...
            {
                if((int)P.find(LeftRule[i]) == -1) P += LeftRule[i];
                if((int)N.find(LeftRule[i]) == -1) N += LeftRule[i];
...
void reachable(string nonTerminals, vector<string>* RRich, vector<string>* LRich, vector<string> RightRule, vector<string> LeftRule)
{
    string W = LeftRule[0];
...
int findChains(string chain, string terminal, string nonTerminal){
    ...
    if(chain.length() > upperLimit) return 0;
    ...
    for(int i = 0; i < entLRules.size(); i++)
    {
        ...
        bufChain = substitution(chain,entRRules[i],chain.find(entLRules[i]), entLRules[i].length()); 
...
int main(int argc, char* argv[])
{
    ...
    {   
        ...
        while(buf != NULL){
            buf = strtok(NULL,">");
            if(buf != NULL)RightRule.push_back(buf);
        }
    }
    ...
    for(int i = 0; i < FinalChains.size(); i++)
    {
        ...
        if(!b) finalch.push_back(FinalChains[i]);
